...
class PlotPrecisionRecall(object):
    def __init__(self,dataset,layerName,orignalDim,L):
        ...
        self.L=L
    def LoadCosineDistance(self):
        self.Distance=np.loadtxt("../data/"+self.dataset+"Original_CosineDistance_"+self.layerName+"_"+str(self.orignalDim)+".txt") 
#         f = file("CosineDistance_"+self.layerName+".npy", "a+b")
#         self.Distance=np.load(f)
#         f.close()
        print "Load Cosine distance of (%s) is OK!" %self.layerName
    ...
    def PlotPrecisionRecall(self):
        ...
        threshold=np.linspace(self.MaxDistance,self.MinDistance,300)
...
#             print samplePointIndex
            ...
            j=self.L+1
            while j<dim:
                temp=self.Distance[j][0:j-self.L].argmax()
                if self.Distance[j][temp]>=D:
                    correspondence[j][temp]=1
                j=j+1
